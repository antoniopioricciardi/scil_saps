...
class SCILEncoderEfficientNet(nn.Module):
    def __init__(self, num_actions=7, freeze_backbone=False, variant='b0'):
        ...
        # Projection layer to create learnable latent space for SupCon
        # This matches NatureCNN's architecture: backbone → fc + ReLU → h
        # Without this, h is raw backbone features with no dedicated latent space
        # self.latent_dim = 512
        # self.fc = nn.Linear(self.repr_dim, self.latent_dim)
    ...
    def forward(self, x):
        ...
        features = self.backbone(x)  # Output: (B, repr_dim, 1, 1)
        features = features.view(features.size(0), -1)  # Flatten to (B, repr_dim)

        # Project to latent space with ReLU (matches NatureCNN architecture)
        # h = F.relu(self.fc(features))
        h = F.relu(features)  # Use raw backbone features as embeddings (no projection)
...
#         # Policy Head (for action prediction - L_pred)
#         # As per paper Figure 1: policy head operates on embeddings e_i directly
#         self.policy_head = nn.Linear(self.latent_dim, num_actions)
